package StarbleTest;
use strict;
use warnings;

use FindBin;
$ENV{CATALYST_CONFIG} = "$FindBin::Bin/conf/starble.conf";
$ENV{CATALYST_DEBUG} = 0;

use Starble::Schema;

my $db_dir    = "$FindBin::Bin/db";
my $db_file   = "$db_dir/starble.db";
my $dsn       = "dbi:SQLite:dbname=$db_file";

sub init_schema {
    my $self = shift;

    if (-e $db_file) {
        unlink $db_file
            or croak("Couldn't unlink $db_file: $!");
    }

    my $schema = Starble::Schema->
        connect( $dsn, '', '', {}, );

    $schema->deploy( { add_drop_table => 1 }, $db_dir );

    $schema->populate(
        'Thing',
        [
            [qw/id uri/],
            [1, 'http://example.com/foo', ],
            [2, 'http://example.com/bar', ],
            [3, 'http://example.com/baz', ],
        ]
    );

    $schema->populate(
        'Star',
        [
            [qw/id ip time session thing/],
            [1, '1.2.3.4', 1, '2000-01-01 00:00:00', 1],
            [2, '1.2.3.5', 2, '2000-01-01 00:00:00', 1],
            [3, '1.2.3.6', 3, '2000-01-01 00:00:00', 1],
            [4, '1.2.3.4', 4, '2000-01-01 00:00:00', 2],
        ]
    );

    $schema->populate(
        'Session',
        [
            [qw/id shorter_id/],
            [1, 1],
            [2, 2],
            [3, 3],
            [4, 4],
        ]
    );

    return $schema;
}

1;
